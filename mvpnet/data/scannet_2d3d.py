...
class ScanNet2D3DChunks(Dataset):
    ...
    def __init__(
        ...
        self.raw_to_nyu40_mapping = read_label_mapping(
            self.label_id_tsv_path, label_from="id", label_to="nyu40id", as_int=True
        )
        self.raw_to_nyu40 = np.zeros(
            max(self.raw_to_nyu40_mapping.keys()) + 1, dtype=np.int64
        )
    ...
    def _load_dataset(self):
        with open(
            osp.join(self.cache_dir, "scannetv2_{}.pkl".format(self.split)), "rb"
        ) as f:
    ...
    def get_rgbd_data(self, data_dict, chunk_points, chunk_box, chunk_mask):
        ...
        pointwise_rgbd_overlap = data_dict["pointwise_rgbd_overlap"].astype(
            np.bool_
        )  # (nb, nf)
        chunk_basepoint_rgbd_overlap = pointwise_rgbd_overlap[
            base_point_mask[base_point_ind]
        ]  # (nbc, nf)
        ...
        selected_frames = select_frames(
            chunk_basepoint_rgbd_overlap, self.num_rgbd_frames
        )
        ...
        for i, frame_id in enumerate(selected_frames):
            ...
            if self.resize:
                if not image.size == self.resize:
                    ...
                    assert (
                        image.size[0] > self.resize[0]
                        and image.size[1] > self.resize[1]
                    )
            ...
            if not np.any(image_mask):
                print(
                    "Invalid depth map for frame {} of scan {}.".format(
                        frame_id, scan_id
                    )
                )
        ...
        nbrs = NearestNeighbors(n_neighbors=self.k, algorithm="ball_tree").fit(
            image_xyz_valid
        )
    ...
    def __getitem__(self, index):
        ...
        scan_id = data_dict["scan_id"]
        assert scan_id == self.scan_ids[index], "Mismatch scan_id: {} vs {}.".format(
        ...
        for _ in range(10):
            ...
            chunk_mask = np.all(
                np.logical_and(
                    xy >= (chunk_min - self.chunk_margin),
        ...
        if chunk_nb_pts < self.nb_pts:
            ...
            crop_pad_choice = np.random.choice(
                chunk_nb_pts, size=self.nb_pts, replace=False
            )
        ...
        if self.num_rgbd_frames > 0:
            ...
            chunk_box = np.hstack(
                [chunk_min - self.chunk_margin, chunk_max + self.chunk_margin]
            )
            out_dict_rgbd = self.get_rgbd_data(
                data_dict, chunk_points, chunk_box, chunk_mask
            )
        ...
        if self.z_rot:
            ...
            if "image_xyz" in out_dict:
                ...
                out_dict["image_xyz"] = Rot.apply(image_xyz.reshape([-1, 3])).reshape(
                    image_xyz.shape
                )
                out_dict["image_xyz"] = out_dict["image_xyz"].astype(
...
def test_ScanNet2D3DChunks():
    ...
    image_dir = osp.join(
        "/home/docker_user/workspace/mvpnet_private/data/ScanNet/scans_resize_160x120"
    )
    ...
    def __getitem__(self, index):
        ...
        for chunk_ind, chunk_box in zip(chunk_indices, chunk_boxes):
            ...
            if self.num_rgbd_frames > 0:
                ...
                out_dict_rgbd = self.get_rgbd_data(
                    data_dict, chunk_points, chunk_box[[0, 1, 3, 4]], chunk_mask
                )
                out_dict.update(out_dict_rgbd)
